...
export function parseManifest(value) {
    ...
    if (!/^rp_[0-9a-z]+_[0-9a-f]{12}$/.test(id))
        corrupt(`恢复点 id 无效：${JSON.stringify(id)}`);
    ...
    if (storage !== 'jj' && storage !== 'blob')
        corrupt('storage 必须是 "jj" 或 "blob"');
    const commitId = optionalString(record, 'commitId');
    if (storage === 'jj') {
        if (commitId === undefined || !/^[0-9a-f]{40}$/.test(commitId))
            corrupt('jj 后端的恢复点必须携带 40 位 commitId');
    }
...
export function parseOperation(value) {
    ...
    if (record.version !== FORMAT_VERSION)
        corrupt(`不支持的操作日志版本 ${String(record.version)}`);
    const id = stringField(record, 'id');
    if (!/^op_[0-9a-z]+_[0-9a-f]{12}$/.test(id))
        corrupt(`操作 id 无效：${JSON.stringify(id)}`);
    ...
    for (const point of [restorePointId, rescuePointId]) {
        if (!/^rp_[0-9a-z]+_[0-9a-f]{12}$/.test(point))
            corrupt(`操作引用的恢复点 id 无效：${JSON.stringify(point)}`);
    }
    const workspace = absoluteString(record, 'workspace');
    const pathsValue = record.paths;
    if (!Array.isArray(pathsValue))
        corrupt('操作日志 paths 必须是数组');
    const paths = pathsValue.map((path) => validateRelativePath(String(path)));
    if (new Set(paths).size !== paths.length)
        corrupt('操作日志 paths 含重复项');
...
function parseEntry(value, path) {
    ...
    if (mode > 0o7777)
        corrupt(`快照 mode 越界：${JSON.stringify(path)}`);
    if (kind === 'file') {
        const blob = stringField(record, 'blob');
        if (!/^[0-9a-f]{64}$/.test(blob))
            corrupt(`快照 blob 哈希无效：${JSON.stringify(path)}`);
        const entry = { kind, blob, size: nonNegativeInteger(record, 'size'), mode };
        return entry;
    }
    if (kind === 'symlink') {
        const target = stringField(record, 'target');
        if (target.includes('\0'))
            corrupt(`快照符号链接 target 含 NUL：${JSON.stringify(path)}`);
...
function hashField(record, key) {
    ...
    if (!/^[0-9a-f]{64}$/.test(value))
        corrupt(`${key} 必须是 64 位小写 hex`);
    return value;
}
function nonNegativeInteger(record, key) {
    const value = record[key];
    if (!Number.isSafeInteger(value) || value < 0)
        corrupt(`${key} 必须是非负安全整数`);
    return value;
}
