...
export class WorkspaceStore {
    ...
    async readManifest(workspace, id) {
        if (!ID_PATTERN.test(id))
            throw new ShadowRewindError('INVALID_RESTORE_POINT_ID', `恢复点 id 无效：${JSON.stringify(id)}`);
        const dir = await this.workspaceDir(workspace);
        let raw;
        try {
    ...
    async putBlob(workspace, hash, content) {
        if (!/^[0-9a-f]{64}$/.test(hash))
            throw new ShadowRewindError('STATE_CORRUPT', `非法 blob 哈希 ${JSON.stringify(hash)}`);
    ...
    async collectGarbage(workspace) {
        ...
        for (const manifest of await this.listManifests(workspace)) {
            for (const entry of Object.values(manifest.entries)) {
                if (entry.kind === 'file' && manifest.storage === 'blob')
    ...
    async assertStorageSeparated(workspace) {
        ...
        if (workspaceReal === storageReal
            ...
            throw new ShadowRewindError('STORAGE_INSIDE_WORKSPACE', `存储目录与工作区重叠：storage=${JSON.stringify(storageReal)} workspace=${JSON.stringify(workspaceReal)}`);
        }
    }
}
// ── 内部工具 ─────────────────────────────────────────────────────────────
