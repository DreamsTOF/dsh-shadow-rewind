...
*/
function traceBaselinePaths(events) {
...
*/
function replayUntil(ops, untilSeqExcl, notes, baseline) {
	const state = /* @__PURE__ */ new Map();
	if (baseline !== void 0) for (const [path, content] of baseline) state.set(path, content);
	for (const op of ops) {
...
*/
function traceRangeDiff(events, fromSeq, toSeq, options) {
	const { ops } = contentOps(events);
...
//#endregion
export { MUTATING_CONTENT_TOOLS, collectTurnIntent, contentOps, parseToolArguments, toolResultError, toolTargetPath, traceBaselinePaths, traceNodes, traceRangeDiff, traceSpans, turnBoundaries };
