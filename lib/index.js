...
var ShadowRewindService = class {
	...
	constructor(ctx, config = {}) {
		...
		this.commandWindows = new CommandWindowRegistry({ canonicalDirectory: (path) => canonicalDirectory(path).catch(() => void 0) });
		...
		installCommandWindowRecorder(ctx, this.commandWindows, () => recorderServices.sessions);
		...
		ctx.inject(["sessions"], (scope) => {
			...
			recorderServices.sessions = scope.sessions;
		});
		ctx.inject([
			"webServer",
			"sessions",
