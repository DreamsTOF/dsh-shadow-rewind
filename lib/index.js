...
import { REWIND_GATE_PATH, REWIND_HTTP_PATH, TurnCheckpointCoordinator, installShadowRewindCommands, installShadowRewindHttp, partitionRunningSessions, sessionEvents } from "./rewind-host.js";
...
var ShadowRewindService = class {
	...
	constructor(ctx, config = {}) {
		...
		installCommandWindowRecorder(ctx, this.commandWindows, () => gateLookup);
		ctx.inject(["agents"], (scope) => {
			const agents = scope.agents;
			gateLookup.get = (id) => agents.get?.(id);
			gateLookup.list = () => agents.list();
		...
		ctx.inject([
			...
			installShadowRewindHttp(scope, this.engine, this.coordinator, this.writeGate, this.commandWindows);
		});
...
//#endregion
export { CHAR_HIGHLIGHT_MAX_CHARS, DEFAULT_EXCLUDES, FORMAT_VERSION, FileReviewService, REWIND_GATE_PATH, REWIND_HTTP_PATH, ShadowRewindEngine, ShadowRewindError, ShadowRewindService, ShadowRewindService as default, TurnCheckpointCoordinator, errorMessage, installShadowRewindCommands, installShadowRewindHttp, isCheckpointSkipCode, lineHighlight, partitionRunningSessions, resolveConfig, sessionEvents, transformFile };
