...
*/
function walkParentLineage(header, lookup, visit, initialSeen) {
	let current = header;
...
var WorkspaceWriteGate = class {
	...
	*/
	...
	async lineageReaches(agent, owner) {
		let reaches = false;
		walkParentLineage(agent.session.header, (parentId) => this.deps.sessions?.get(parentId)?.session.header, (parentId) => {
			if (parentId === owner) {
				reaches = true;
				return false;
			}
...
//#endregion
export { DEFAULT_READONLY_TOOLS, WorkspaceWriteGate, installWriteGateHost, walkParentLineage };
