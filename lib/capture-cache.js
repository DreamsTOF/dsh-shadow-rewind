...
export async function readCaptureCache(path) {
    try {
...
/** 原子写回缓存。 */
export async function writeCaptureCache(path, cache) {
 ...
 * 影子仓库被外部清理。缺失即无操作。
 */
export async function clearCaptureCache(path) {
    try {
        await unlink(path);
    }
    catch (error) {
...
function isCache(value) {
    ...
    return record.version === 1
        ...
        && record.checksum === checksumOf(record.paths);
}
/** 从扫描事实生成缓存记录（blob 由调用方在实际读到内容后补充）。 */
export function cacheEntryOf(file, blob, target) {
    return {
        kind: file.kind,
        size: file.size,
        mode: file.mode,
...
export function cacheMatches(cached, file) {
...
//# sourceMappingURL=capture-cache.js.map