...
function attributePaths(options) {
	...
	for (const change of options.changes) {
		...
		let owner;
		if (owners.size === 0) owner = { kind: "unknown" };
		else if (others.length === 0) owner = { kind: "target" };
		else if (owners.size === 1 && singleOther !== void 0 && singleOther !== "unknown") owner = {
			kind: "session",
			sessionId: singleOther
		};
		else if (owners.size === 1) owner = { kind: "unknown" };
		else owner = { kind: "multi" };
		result.set(change.path, { owner });
	}
	return result;
}
