...
export class ShadowRewindEngine {
    ...
    async captureTree(workspace, options) {
        ...
        const verifyContent = async (path, blob) => {
            if (this.effectiveBackend === 'blob')
                return this.store.blobExists(workspace, blob);
        ...
        try {
            ...
            try {
                ...
                for (const stale of sameSession.slice(this.config.maxTurnCheckpointsPerSession)) {
                    if (signal.aborted)
                        break;
                    if (await this.isReferencedByRecovery(workspace, stale.id))
                        continue;
    ...
    async verifyRestored(workspace, manifest, paths, signal) {
        ...
        for (const path of paths) {
            ...
            if (process.platform !== 'win32' && Number(info.mode & 4095n) !== entry.mode) {
...
export function isCheckpointSkipCode(code) {
