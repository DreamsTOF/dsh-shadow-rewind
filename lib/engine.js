...
function resolveConfig(config) {
	const storageDir = config.storageDir?.trim() !== "" ? String(config.storageDir) : join(process.env.DSH_HOME ?? join(homedir(), ".dsh"), "shadow-rewind", "v1");
	const mode = config.turnCheckpointMode ?? DEFAULTS.turnCheckpointMode;
	if (mode !== "off" && mode !== "legacy" && mode !== "jj") throw new ShadowRewindError("INVALID_CONFIG", "turnCheckpointMode 必须是 off、legacy 或 jj");
...
var ShadowRewindEngine = class {
	...
	*/
	...
	constructor(config = {}) {
		...
		} else this.effectiveBackend = this.config.turnCheckpointMode === "off" ? "blob" : this.config.turnCheckpointMode === "jj" ? "jj" : "blob";
	...
	*/
	async captureTree(workspace, options) {
		...
		if (options.mode === "persist") {
			...
			else for (const [path, content] of captured.newContent) {
	...
	*/
	...
	async createTurnCheckpoint(options) {
		...
		try {
			...
			try {
				...
				const duplicate = existing.find((manifest) => manifest.kind === "turn" && manifest.sessionId === options.sessionId && manifest.turn === options.turn && manifest.turnStartSeq === options.turnStartSeq);
				if (duplicate !== void 0) {
				...
				await this.store.deleteTurnSkip(workspace, options.sessionId, options.turn, options.turnStartSeq).catch(() => void 0);
				const sameSession = [...existing, manifest].filter((point) => point.kind === "turn" && point.sessionId === options.sessionId).sort((left, right) => right.createdAt - left.createdAt || right.id.localeCompare(left.id));
	...
	async findTurnCheckpoint(options) {
		...
		const workspace = await canonicalDirectory(options.cwd);
		const found = (await this.store.listManifests(workspace)).find((manifest) => manifest.kind === "turn" && manifest.sessionId === options.sessionId && manifest.turn === options.turn && (options.turnStartSeq === void 0 || manifest.turnStartSeq === options.turnStartSeq));
	...
	async createLocked(workspace, options) {
		const tree = await this.captureTree(workspace, {
			mode: "persist",
			message: options.kind === "turn" ? `turn ${String(options.turn)} checkpoint (session ${options.sessionId ?? "?"})` : options.kind === "rescue" ? `rescue before restoring ${options.parentRestorePoint ?? "?"}` : options.label ?? "user restore point",
			signal: options.signal
		});
		const manifest = {
			version: 1,
	...
	*/
	...
	async restorePaths(workspace, manifest, paths, signal) {
		...
		const skippedSet = new Set(manifest.skippedPaths.map((skip) => skip.path));
		const deletions = paths.filter((path) => manifest.entries[path] === void 0 && !skippedSet.has(path)).sort((left, right) => depthOf(right) - depthOf(left));
