...
function fsChangeShape(file) {
	...
	if (diff.newText === "" && diff.oldText !== "") return {
		kind: "deleted",
		dir: false
	};
	return null;
...
async function writeRescueCopy(rescueDir, path, content) {
	try {
		...
		await writeFileAtomic(join(rescueDir, name), content, { mode: 384 });
		return true;
	} catch {
		return false;
	}
...
async function applyFsChange(cwd, file, action, shape, rescueDir) {
	const diff = file.diffs[0];
	try {
		...
		if (inspected.state !== sourceState) return {
			path: file.path,
		...
		if (shape.kind === "mode") {
			...
			const state = await fsFileState(cwd, file.path);
			if (!state.exists || normalizeNewlines(state.text) !== normalizeNewlines(diff.newText)) return {
				path: file.path,
				state: "conflict",
				changed: false,
		...
		if (removes) {
			...
			if (!(state.exists && expected !== null && normalizeNewlines(state.text) === normalizeNewlines(expected))) return {
				path: file.path,
				state: "conflict",
				changed: false,
				reason: "file changed while the operation was being prepared"
...
async function applyOne(cwd, file, action, rescueDir) {
	const fsShape = fsChangeShape(file);
