...
async function scanWorkspace(cwd, options) {
	...
	for (const scanned of paths) {
		...
		for (let depth = 1; depth < segments.length; depth += 1) nonEmpty.add(segments.slice(0, depth).join("/"));
	}
	return {
		root,
		paths,
		skipped,
		emptyDirs: visitedDirs.filter((dir) => !nonEmpty.has(dir.path)).sort((left, right) => left.path < right.path ? -1 : left.path > right.path ? 1 : 0)
	};
}
