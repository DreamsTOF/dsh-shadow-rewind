...
import { chmod, lstat, mkdir, open, opendir, rename, rm, rmdir, symlink } from "node:fs/promises";
import { randomUUID } from "node:crypto";
import { dirname, join } from "node:path";
import { execFile, execFileSync } from "node:child_process";
//#region src/jj-backend.ts
/**
...
var ShadowJj = class {
	...
	*/
	async initialize(signal) {
		...
		if (!exists) {
			if (await pathExists(join(this.repoDir, MIRROR_DIR))) throw new ShadowRewindError("JJ_REPO_LOST", `影子仓库丢失（残留镜像目录）：${JSON.stringify(this.repoDir)}`);
			await runJj([
				"git",
				"init",
				this.repoDir
	...
	async capture(paths, newContent, newLinks, message, options) {
		...
		try {
			for (const entry of paths) {
				...
				if (fileName === void 0 || fileName === "" || segments.some((s) => s === "" || s === "." || s === "..")) throw new ShadowRewindError("INVALID_PATH", `镜像路径不是规范形式：${entry.path}`);
				...
				if (writtenBytes > options.maxNewBytes) throw new ShadowRewindError("TURN_CHECKPOINT_NEW_CONTENT_LIMIT", `本次自动检查点需新写 ${String(writtenBytes)} 字节，超出上限 ${String(options.maxNewBytes)}`);
			...
			if (error instanceof ShadowRewindError) await runJj(["restore"], this.repoDir).catch(() => void 0);
			throw error;
		}
		const commitId = (await runJj([
			"log",
			"--no-graph",
			"-r",
			"@-",
			"-T",
			"commit_id"
		], this.repoDir, signal)).trim();
		if (!/^[0-9a-f]{40}$/.test(commitId)) throw new ShadowRewindError("STATE_CORRUPT", `jj 未返回有效的 commit id：${JSON.stringify(commitId)}`);
		return {
			commitId,
			writtenBytes
		};
	...
	*/
	async readSnapshot(commitId, path, signal) {
		...
		try {
			...
			if (!isJjFailure(error)) throw error;
		}
		try {
			return await runJjBuffer([
				"cat",
			...
			if (isJjFailure(error)) return null;
			throw error;
		}
	}
};
...
async function pruneExtra(mirrorRoot, root, wanted, signal) {
	const wantedDirs = /* @__PURE__ */ new Set();
	...
	while (stack.length > 0) {
		...
		try {
			for await (const entry of handle) {
				...
				if (!(isDir ? wantedDirs.has(rel) : wanted.has(rel))) {
	...
	for (;;) {
		...
		if (parent === current || current === mirrorRoot) return;
		current = parent;
	}
}
//#endregion
export { ShadowJj, jjAvailable };
