 ...
 */
export async function ensureSafeParents(root, target) {
    await walkSafeParents(root, target, true);
}
/** 校验已有父目录（不创建缺失目录），apply 前的计划复核用。 */
export async function assertSafeParents(root, target) {
...
export async function replaceRegularFile(path, content, mode) {
...
/** 用同级临时名把一个路径替换为符号链接。 */
export async function replaceSymbolicLink(path, target) {
    const temporary = join(dirname(path), `.${randomUUID()}.shadow-rewind.tmp`);
    try {
 ...
 */
export async function removeRestoreTarget(path) {
    let info;
    try {
...
export async function pruneEmptyParents(root, start) {
...
export async function safeFileNames(path) {
    try {
...
export async function safeDirectoryNames(path) {
    try {
        const entries = await readdir(path, { withFileTypes: true });
