...
| **终端写盘审计** | 检查点轮 diff 兜底    | 文件 / 空目录 / 权限位               | 轮尾卡片 + 轮中 live 条 + 侧边栏       |
...
2. **回退入口**：每条直发用户消息下出现「恢复到发送之前」；对话框预览将恢复的文件清单与快照时显式跳过的路径，并提供两种落点：
   ...
   - **只恢复文件** —— 当前对话不动。
3. **恢复安全闸**：恢复前自动 rescue 备份 → 事后逐路径哈希/类型/权限验证 → 失败自动回滚——「要么恢复成功，要么整体回到操作前」的两态翻转，绝不留半恢复状态；流程性检查（计划过期、会话绑定）只作软警告，选择权通过冲突弹窗交给用户。
...
- **聊天轮尾卡片**：每轮结束后展示「Edited N files · +M −K」、**两个并列回滚按钮**——「回滚本轮 AI 更改」（只撤销内容型工具的写入，保留你本轮的手动小修改；终端写盘走各自文件行的单文件撤销）与「回滚到上一轮检查点」（整树恢复，覆盖本轮内一切变更）、文件行（点击深链进侧边栏对应轮的差异）；**每个文件可单独撤销/重做该文件在本轮的全部改动**；文件行**悬停即显完整 diff 浮层**（宽度与卡片框对齐、视口内自动翻转）；
...
- **删除安全网**：任何走删除分支的 fs 撤销（新增撤销 / 删除重做）先把即将删除的内容落一份可找回副本（目录有数量上限），落盘失败则拒绝删除；
- **多会话归属标签**：轮配对与 live 清单按检查点窗口网格给每条变更标归属（本会话 / 其它会话 / 双方 / 未知）——归属只是勾选清单的**建议标签**，不裁决可见性；多主路径从单会话默认勾选中排除（引擎级整树恢复仍可还原）。两条保守边界：会话在自己开轮捕获之前的写盘归属前一个窗口；轮间手动/外部修改不属于任何轮。

## 四、多会话并行

同一个项目目录跑多个会话时，并行写入不设限制；恢复按归属勾选：

- **归属标签**：恢复预览里每条变更按检查点窗口标注归属——本会话 / 其它会话 / 双方都改过 / 来源不明——默认只勾选本会话路径，也可手动勾选任意路径，「全部选中」即整树恢复；
- **归属是建议不是裁决**：标签来自影子仓库里所有会话的检查点时间线（检查点在回合开始捕获 → 窗口 [S*j, S*{j+1}) 的写者就是 S_j 的会话）。勾选权在用户，错了最多误导、不破坏数据；
- **恢复不等空闲**：恢复不再被运行中的会话阻塞。安全网是恢复前的自动 rescue 备份 + 逐路径哈希/类型/权限验证 + 失败自动回滚；
...
| 端点                                            | 用途                                                                                                                                        |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET/POST /shadow-rewind`                       | 消息回退预览（`?sessionId&messageSeq`）、按轮快照预览（`?sessionId&turn`）、`paths=[...]` 铸造勾选子集计划、`POST` 计划 id 执行恢复（确认由弹窗承担） |
| `GET /shadow-rewind/fs-changes?sessionId`       | 批量返回该会话所有轮次的终端写盘变更（行数、权限位、目录标记、意图摘要、degraded 标注、`rev` 数据版本、窗口归属标签）                         |
...
| `POST /shadow-rewind/restore-undo`              | 撤销该工作区最近一次恢复（进程内单次 undo；body `{sessionId\|cwd, mode?: probe\|apply, force?, paths?}`——`probe` 只读比对冲突，`force` + `paths` 为弹窗确认后的强制/子集回滚） |
...
- **归属网格**：跨会话窗口归属标签（target/它会话/multi）、勾选式子集恢复、计划与检查点错配拒绝（PLAN_STALE）、带 paths 的子集计划铸造；
- **undo 可靠性**：部分成功保留记录可重试、撤销递增数据版本、补偿回滚不覆盖撤销槽、undo 引用的 rescue 点不被配额修剪；
- **终端写盘端点**：轮次配对行数与 `rev`、权限位透传、轮配对与 live-tail 的跨会话归属剔除；
- **混沌套件（固定种子可复现）**：双会话交错的归属标签与整树恢复收敛、检查点淘汰+GC 压力下存活检查点全部可恢复、双会话随机操作/恢复/淘汰全搅拌（每次恢复后与检查点零差异）、轮末检查点三例（轮末冻结树隔离轮后写盘 / 协调器经 `turn/end` 事件捕获 / 轮起轮末淘汰按相位互不挤占）；7 会话（A-G）不对称轮数（5/2/7/3/6/1/4）+ 两种相反完成顺序下「每会话每轮变更被精确识别」的逐条断言。

## 边界（当前版本）

- 恢复的是**文件内容 + 权限位 + 空目录**：不回滚任何 VCS 历史（工作区 git 的提交不受影响）；非空目录不单独立条目（由子条目在恢复时隐式重建）；符号链接仅在 POSIX 环境验证；
- **恢复撤销是进程内单次 undo**：重启即失效（持久兜底是恢复时自动创建的 rescue 备份点）；只保留最近一次，再次恢复会替换；撤销冲突经三选项弹窗交用户裁决（拒绝 / 强制覆盖 / 只回滚正常部分 + 二次回滚）；
...
- 终端写盘不再做命令级监听与归因（哪条命令写的不可知），清单由检查点轮 diff 兜底：轮末检查点之后的轮间手动/外部修改不属于任何轮；同一路径被多会话触碰时归属标 multi、不默认勾选；轮内创建又删除的临时文件两端检查点都不含、不可见；
- Windows 上权限位语义受限（只读位有效）；
- 跨机共享存储未设计（锁判活按单机 pid）；
- Code Mode 录制按结果形状 `{path, before, after}` 识别嵌套修改，并限定在文件工具名白名单内（write/edit/str_replace_editor）。
