...
function runJj(args: readonly string[], cwd: string, signal?: AbortSignal): Promise<string> {
...
function runJjBuffer(args: readonly string[], cwd: string, signal?: AbortSignal): Promise<Buffer> {
  return new Promise((resolvePromise, reject) => {
    execFile('jj', [...args], {
      cwd,
      encoding: 'buffer',
...
function isJjFailure(error: unknown): boolean {
...
export class ShadowJj {
   ...
   */
  async capture(
    paths: readonly ScannedPath[],
    newContent: ReadonlyMap<string, Buffer>,
    newLinks: ReadonlyMap<string, string>,
    message: string,
    options: { readonly maxNewBytes: number; readonly signal?: AbortSignal },
  ): Promise<{ readonly commitId: string; readonly writtenBytes: number }> {
   ...
   */
  async readSnapshot(commitId: string, path: string, signal?: AbortSignal): Promise<Buffer | null> {
    await this.initialize(signal)
...
async function writeMirrorFile(target: string, content: Buffer, mode: number): Promise<void> {
 ...
 * 子目录会被误删。
 */
async function pruneExtra(mirrorRoot: string, root: string, wanted: ReadonlySet<string>, signal?: AbortSignal): Promise<void> {
