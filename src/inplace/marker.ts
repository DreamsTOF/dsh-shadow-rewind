...
export const INPLACE_MARKER_PLUGIN = 'dsh-shadow-rewind'

export const INPLACE_MARKER_SOURCE = {
  kind: 'plugin' as const,
  plugin: INPLACE_MARKER_PLUGIN,
} as const

/** 空内容标记（append 时禁止复用同一引用，避免宿主修改污染常量）。 */
export const INPLACE_MARKER_CONTENT: readonly unknown[] = []

/** 构造一条可 append 的标记消息体（每次新对象）。 */
export function buildInPlaceMarker(): {
  readonly content: readonly unknown[]
  readonly source: { readonly kind: 'plugin'; readonly plugin: string }
} {
  return {
    content: [...INPLACE_MARKER_CONTENT],
    source: { kind: 'plugin', plugin: INPLACE_MARKER_PLUGIN },
  }
}
