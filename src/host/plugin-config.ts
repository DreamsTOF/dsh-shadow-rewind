...
export async function loadSchemastery(): Promise<SchemaModule | null> {
  try {
    const mod = (await import('@deepseek-ai/schemastery')) as unknown
    const module = mod as SchemaModule & { default?: SchemaModule }
    const resolved = typeof module?.object === 'function' ? module : module?.default
    return resolved !== undefined && typeof resolved.object === 'function' ? resolved : null
  } catch {
    return null
  }
}
...
export async function createConfigSchema(): Promise<unknown> {
  const Schema = await loadSchemastery()
  if (Schema === null) return null
  const number = (value: number, description: string): unknown =>
    Schema.number().default(value).description(description)
...
export function cleanConfigPatch(patch: Record<string, unknown>): Partial<ShadowRewindConfig> {
  ...
  if (patch.turnCheckpointTrust !== undefined) {
    if (patch.turnCheckpointTrust !== 'fast' && patch.turnCheckpointTrust !== 'strict') {
      throw new Error('turnCheckpointTrust 必须是 fast 或 strict')
    }
    clean.turnCheckpointTrust = patch.turnCheckpointTrust
  }
  if (patch.excludePatterns !== undefined) {
    if (!Array.isArray(patch.excludePatterns)
      || !patch.excludePatterns.every((item): item is string => typeof item === 'string' && item.trim() !== '')) {
      throw new Error('excludePatterns 必须是非空字符串数组')
    }
    clean.excludePatterns = patch.excludePatterns
  }
  return clean as Partial<ShadowRewindConfig>
}
