...
export class BeforeJournal {
  private readonly store: WorkspaceStore
  private readonly config: ResolvedShadowRewindConfig
  /** `${workspace}\0${sessionId}\0${path}` → 最近一次已知内容（recheck 增量判定的单一事实源）。 */
  private readonly lastKnown = new Map<string, string | null>()
  /** `${workspace}\0${sessionId}` → 跟踪路径集（进程内缓存；首条消息时从磁盘重建）。 */
  private readonly trackedCache = new Map<string, Set<string>>()
  /** 每会话单调时钟：同毫秒提交保持捕获顺序。 */
  private readonly lastEntryTime = new Map<string, number>()
  ...
  private sessionDir(workspace: string, sessionId: string): Promise<string> {
    return this.store.workspaceDir(workspace).then((dir) => join(dir, 'before-journal', safeSegment(sessionId)))
  }

  private knownKey(workspace: string, sessionId: string, path: string): string {
  ...
  async record(
    workspace: string,
    sessionId: string,
    entry: Omit<BeforeJournalEntry, 'time'>,
  ): Promise<void> {
    ...
    const record: BeforeJournalEntry = { ...entry, time }
    ...
    if (tracked !== undefined) tracked.add(entry.path)
  }
  ...
  async earliestAfter(
    ...
    sessionId: string,
    afterSeq: number,
  ): Promise<ReadonlyMap<string, BeforeJournalEntry>> {
    ...
    const anchors = anchorNames
      ...
      .filter((value) => Number.isSafeInteger(value) && value >= afterSeq)
      .sort((left, right) => left - right)
    const earliest = new Map<string, BeforeJournalEntry>()
    for (const anchor of anchors) {
      ...
      for (const file of files) {
        ...
        let entry: BeforeJournalEntry
        try {
          entry = JSON.parse(await readFile(join(sessionDir, String(anchor), file), 'utf8')) as BeforeJournalEntry
        } catch {
          continue // 损坏记录静默跳过（同 dsh-rewind 的 readEntry 语义）
        }
        ...
        if (existing === undefined
          || entry.anchorSeq < existing.anchorSeq
          || (entry.anchorSeq === existing.anchorSeq && entry.time < existing.time)) {
   ...
   */
  async trackedPaths(workspace: string, sessionId: string): Promise<Set<string>> {
    ...
    for (const anchor of anchorNames) {
      ...
      for (const file of files) {
        if (!file.endsWith('.json')) continue
        try {
          const entry = JSON.parse(await readFile(join(sessionDir, anchor, file), 'utf8')) as BeforeJournalEntry
   ...
   */
  async prune(workspace: string, sessionId: string): Promise<readonly number[]> {
    const now = Date.now()
    ...
    const anchors = anchorNames
      ...
      .sort((left, right) => right - left) // 新 → 旧
    const stale = anchors.slice(MAX_ANCHOR_GROUPS)
    const pruned: number[] = []
    for (const anchor of stale) {
      await rm(join(sessionDir, String(anchor)), { recursive: true, force: true })
      pruned.push(anchor)
    }
    return pruned
  }
