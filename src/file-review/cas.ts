...
export function restoreNewlines(text: string, crlf: boolean): string {
  return crlf ? text.replace(/\n/g, '\r\n') : text
}

/** 从文件字节判断行尾风格。 */
export function crlfStyle(bytes: Uint8Array): boolean {
  return Buffer.from(bytes).toString('utf8').includes('\r')
}

/** 统一内容比较：两个文件字节在 LF 归一后是否等价。 */
export function contentMatches(currentBytes: Uint8Array, expectedBytes: Uint8Array): boolean {
  return normalizeNewlines(Buffer.from(currentBytes).toString('utf8'))
    === normalizeNewlines(Buffer.from(expectedBytes).toString('utf8'))
}
