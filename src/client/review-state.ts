...
function rowKey(sessionId: string, path: string): string {
...
export function reviewRowOf(sessionId: string, path: string): ReviewRowState | undefined {
  return rows.get(rowKey(sessionId, path))
}
 ...
 */
export function nextReviewAction(sessionId: string, path: string): 'undo' | 'redo' {
