 ...
 */
export function matchPendingRows(rows: readonly PendingRowLike[], steering: readonly SteeringItemLike[]): readonly (string | null)[] {
  const matched: (string | null)[] = []
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i]
    const item = steering[i]
    matched.push(row !== undefined && item !== undefined && row.text === (item.text ?? '') ? item.id : null)
  }
  return matched
}
 ...
 */
export function retractSpan(steering: readonly SteeringItemLike[], targetId: string): readonly string[] {
