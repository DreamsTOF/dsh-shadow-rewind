...
const listeners = new Set<OpenAuditListener>()

/** 请求打开审查界面并展开给定路径。 */
export function openAuditOverlay(paths: readonly string[]): void {
  for (const listener of listeners) listener(paths)
}

/** 订阅打开请求（live 条挂载时注册；返回解绑函数）。 */
export function subscribeOpenAudit(listener: OpenAuditListener): () => void {
